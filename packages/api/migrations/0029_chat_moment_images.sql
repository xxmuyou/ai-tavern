...
-- Register the chat moment workflow in the RunningHub catalog (0028) so the
-- provider's DB lookup resolves it and admin can fill in the real ids. Real
-- workflow_id / prompt_node_id are seeded later from config; placeholder for now.
...
VALUES
  ('wf_moment', 'WF_MOMENT - chat scene moment (create)', 'create', '', '', NULL,
   'ckpt_name', NULL, 1, 30, unixepoch() * 1000, NULL);
