...
**2026-06-16 mode boundary：** 本 spec 只定义 Web scene immersion 和 invite arrival。根据
[`spec-040`](./spec-040-chat-scene-talk-story-modes.md)，`scene_id` 只表示物理场景；是否进入剧情由
`chat_mode` 决定。Invite arrival 只更新当前位置，不强制切换 Story mode。
...
用户点击到达后只更新物理 scene，当前 `chat_mode` 保持不变。
...
4. 点击“立即到达”：
   ...
   - 保持当前 `chatMode`，不因为 invite arrival 自动切 Story。
   - 追加本地转场 narration。
...
所有 Web chat 内 scene 切换共用同一套转场视觉；确认浮层只用于 invite accepted。story transition 可复用转场动画，但不进入 pending arrival 流程，也不改变 invite arrival 的 mode 语义。
...
- 旧文档中把 `scene_id` 视作 Story mode 开关的描述由 spec-040 取代；本 spec 的 pending arrival 只改变 scene。
...
- 点击到达后背景、`scene_id`、转场 narration、后续 prompt scene 均更新，当前 `chat_mode` 不变。
