...
| `TASK_QUEUE_MAXED` | RunningHub 账户/工作区当前可接受任务数已满，task 尚未创建 | `provider_capacity` → 内部排队等待，不落 `failed` |
...
#### B.5.2 RunningHub 并发上限与内部排队（2026-06-18）

- Runtime 设置项：`image_gen.runninghub_max_active_tasks`，env fallback 为 `RUNNINGHUB_MAX_ACTIVE_TASKS`，默认 `3`；Admin Settings 可直接修改 DB override，无需重新部署。
- Active task 口径：`image_generation_jobs.provider = 'runninghub' AND status = 'processing' AND provider_task_id IS NOT NULL`。只有已拿到 RunningHub `taskId` 的任务占用上限；尚未提交成功的本地等待 job 不计入。
- 所有 `image_generation_jobs` 生图入口在调用 RunningHub create 前共用同一个 capacity guard。若 active task 达到上限，后端不调用 RunningHub、不标 failed、不释放积分，而是保持非终态并延迟重新入队。
- 本地等待状态写入 `error_code = provider_queue_wait`、`error_message = Queued`；Capture Moment 状态接口额外返回 `queue_reason = provider_capacity`、`status_label = Queued`，前端显示 `Queued…` 并继续轮询。
- Cloudflare Queue consumer 的 `max_concurrency = 3` 只是入口节流，不能代表 RunningHub 真实任务并发，因为 RunningHub create 返回 taskId 后任务仍在远端异步执行；真正的上限以 provider submit 前的 active-task 统计为准。
- 若 RunningHub create 仍返回 `TASK_QUEUE_MAXED`（例如 RunningHub 页面手动任务或并发竞态），provider 将其归类为 `provider_capacity`，processor 走同一套延迟排队逻辑。
...
- **容量等待**：RunningHub 未创建 task 的容量拒绝（如 `TASK_QUEUE_MAXED`）不算失败；本地延迟排队，用户侧显示 `Queued…`
...
6. **价格 / 并发**（✅ 已核算）：RunningHub 按 GPU 秒计费——标准 24G **0.2 RH币/秒**、Plus 48G 0.4 RH币/秒（[API 文档](https://www.runninghub.ai/runninghub-api-doc-en/doc-8287463)）。本项目 workflow 目标单次 ≤30 秒（§性能），故单图约 **6~12 RH币 ≈ <$0.01**（注册即送 1000+ RH币、$9.99 套餐约 5 万币量级）。据此 spec-021 用户侧定价 **40 积分/图（≈$0.04，兑换率 $1=1000 积分）**，对真实成本仍有 4 倍以上毛利，同时比早期 50 积分更友好。应用侧并发上限由 `image_gen.runninghub_max_active_tasks` / `RUNNINGHUB_MAX_ACTIVE_TASKS` 控制，默认 `3`，可在 Admin Settings 调整。
