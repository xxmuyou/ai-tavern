...
dev/prod 的 Web 前端必须只通过 `dev.aiappsbox.com` / `aiappsbox.com` 暴露；worker 通过 `dev.aiappsbox.com/api/*` / `aiappsbox.com/api/*` 路由暴露（见 `infra/cloudflare/wrangler.jsonc`）。Pages 自动生成的预览域名不能作为产品入口、OAuth 回跳或 Stripe return URL。
...
| 路径 / 资源 | 操作 |
|---|---|
| **Cloudflare DNS** | 使用 Pages Custom domains 自动管理 Web 域名；仅保留邮件相关 TXT 记录（见 §实施步骤 1） |
...
- **DNS**：删除 Cloudflare DNS 中的自定义域记录（10 分钟内全球传播）；前端入口不可回退到 Pages 自动生成域名
...
- **wrangler.jsonc**：保持 `ALLOWED_ORIGINS` / `AUTH_SUCCESS_URL` 等指向 `aiappsbox.com` / `dev.aiappsbox.com`，不得回退到 Pages 自动生成域名
