...
portrait_create 基础立绘（spec-022, txt2img）、**portrait_variation 六表情立绘**（spec-020, img2img）、
chat_moment 瞬间图（spec-027, 场景图）、profile_outfit 换装（spec-030, draft，仅在 codex worktree）。

矛盾点在 **portrait_variation**：每角色用 img2img 生 6 张表情立绘（neutral/warm/playful/guarded/tense/annoyed），
...
**本 spec 修订前序决策：**
- 修订 spec-020：**退役 portrait_variation 表情立绘生成**（停写保数据），情绪改为只驱动 UI。
- 修订 spec-027 的"不做抠图"非目标：**瞬间图改为用抠好的透明角色作合成/参考源**，提升跨场景角色一致性。
- spec-030（profile outfit）**后端配置保留**（runninghub workflow / image_workflows 中的 `profile_outfit` 不动），**前端本轮不实现**。
...
### 目标
- **主形象走干净底图**：portrait_create 输出"soft studio / 干净渐变 / bokeh 背景、主体居中"的展示图，直接展示、不抠图。
- **新增可配置的抠图 workflow 接口**：`companion_cutout` + 新 `cutout` mode，聊天时按需对主形象做 AI matting（RunningHub），结果缓存。
- **瞬间图消费抠图角色**：chat_moment 用透明角色作 img2img/参考源合成进场景，角色跨场景一致。
- **情绪只驱动 UI**：复用每轮 `emotion` 信号 → tint / emoji / 氛围 / 轻动效，不再逐条重生成立绘。
- **退役 portrait_variation**：路由/消费/admin/config/前端消费全部下线；历史数据停写保留。
...
- ❌ 本轮不做 profile_outfit 前端（spec-030）；但**其后端配置保留不动**。
...
|---|---|---|---|---|
| `portrait_create` | 主形象（**干净/影棚背景**） | `create` | prompt(+可选上传) → 干净背景展示图 | 改 prompt 约束 |
| `companion_cutout` | **抠图（AI matting）** | **新增 `cutout`** | base art(`source_art_url`) → 透明 PNG 角色 | **新增** |
| `chat_moment` | 瞬间/场景图 | `create`/`variation` | 透明角色作参考/合成 + 场景 prompt → 场景图 | 改：吃 cutout |
| ~~`portrait_variation`~~ | ~~六表情立绘~~ | — | — | **退役（停写保数据）** |
| `profile_outfit` | 换装 | `variation` | — | **后端配置保留，前端暂不实现** |

数据流：
```
companions.art_url (portrait_create, 干净背景)
   └─(首次按需→缓存 art_cutout_key)→ companion_cutout (matting) → 透明角色
                                                 └─→ chat_moment img2img/参考源 → 场景瞬间图
```
...
-- 标记 portrait_variation 退役：art_emotions / companion_art_jobs 保留不删，停止写入（迁移内注释说明）
```
...
### 端点
- 抠图**无独立公开端点**：由 chat_moment 生成前内部触发（`art_cutout_key` 为空时懒生成）。
...
- [index.ts:68-89](../../packages/api/src/image-gen/index.ts#L68-L89) `getImageGenProvider`：为 `companion_cutout` 增一支
  （`cfg.wfCutoutProvider`，空回落 default → mock），与现有 `chat_moment` 分流写法一致。
...
- `runninghub-provider.ts`：`generateCutout()` 把 base art 推到 `companion_cutout.loadImageNodeId`，
...
**P1 地基（先成立新体验）**
1. portrait_create prompt 改干净底图 + 端到端验证 alpha 存储不被 flatten。
...
5. chat_moment 改吃 `art_cutout_key`（空则先抠）。
6. 前端 PortraitBar 单图 + UI 情绪 + 轻动效。

**P2 收尾**
7. 退役 portrait_variation：路由 410、移除消费/prompts/expression-prompts/admin、config 停用、前端 hook/解锁清理。
8. profile_outfit：后端配置保留不动，前端暂不实现（无改动，仅确认未被本 spec 误删）。
9. 画廊改版为"主形象 + 瞬间图集"。

---

## 验证
1. **portrait_create 干净底图**：生成主形象，背景干净、主体居中、利于抠图。
2. **抠图链路**：触发 `companion_cutout` 输出透明 PNG、alpha 保留、缓存命中（二次不重抠）；`art_url` 变更后失效重抠。
...
5. **portrait_variation 下线**：`/companions/{id}/emotion-art/*` 返回下线；queue 不处理该类型；admin 不暴露 expression_prompts；历史 `art_emotions` 仍在库。
6. **静态检查**：
...
- 抠图/`art_cutout_key`：列可空，chat_moment 回落用原始 base art 作源即恢复 spec-027 行为。
- portrait_variation：数据未删，重新激活路由/config/消费即可恢复。
- `companion_cutout` 未配置：生产返回明确 `provider_not_configured`，dev/mock 继续通过测试。

---

## 依赖
- spec-006（chat）、spec-020（emotion-art，**本 spec 退役其 portrait_variation 部分**）、
...
## 待定（落库前需用户给定，不得脑补）
1. **companion_cutout 的 RunningHub workflow**：matting workflow id + loadImage 节点 id + matting 模型（BiRefNet / RMBG-2.0 / BEN2）。
2. **chat_moment 合成方式**：(a) alpha 合成+协调 还是 (b) IP-Adapter/参考重绘？需对应 RunningHub workflow id/节点。
