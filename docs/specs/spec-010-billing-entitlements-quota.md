...
- 401 `auth_required`
- 500 `billing_config_missing`：prod 缺 `STRIPE_SECRET_KEY`、`STRIPE_PRICE_PRO_MONTHLY`、success/cancel URL
...
- `STRIPE_PRICE_PRO_MONTHLY`：public var，不是 secret
...
- checkout 创建或复用 Stripe customer，且只使用 `STRIPE_PRICE_PRO_MONTHLY`。
