...
// Request
{
  ...
  "quick_action": null                  // 可选（spec-037）：{ "type": "gift", "item_id": "coffee" | "flowers" }
}
...
> **聊天快捷动作（spec-037）：** 当请求带合法 `quick_action` 时，后端先创建已完成 activity、写入 memory，并应用固定关系加成。咖啡仅当前 scene 的 name/mood/tags 含 `cafe` 或 `coffee` 时允许；鲜花任意 scene 允许；同一用户对同一 companion 的同一 item 有 6 小时冷却。不扣 credits。成功后 SSE 额外事件：
> ```json
> // event: quick_action_result
> { "ok": true, "item_id": "coffee", "activity_id": "act_xxx", "memory_id": null, "cooldown_until": null }
> ```
> 冷却或校验失败会在开始流式前以 422 JSON 返回。

> **邀约换场景（spec-036/037）：** 当请求带合法的 `invite_scene_id`（必须是已解锁场景，见下方 invite-targets），后端在 prompt 注入一段"用户邀请你前往 {场景}"的指令；本轮仍以**当前** `scene_id` 生成回复。回复结束后跑一次接受/拒绝判定，并通过 SSE 额外事件 `invite_result` 推送；若本轮来自 active activity 且对方接受，后端自动完成该 activity：
> ```json
> // event: invite_result
> { "accepted": true, "scene_id": "tavern", "scene_art_url": "...", "reason": "...", "activity_completed": true }
> ```
