...
event: invite_result
data: {"accepted": true, "scene_id": "restaurant", "scene_art_url": "scenes/restaurant.png", "reason": "..."}
  ...
  "items": [
    {
      ...
      "art_url": "https://...",
      "preferred_scenes": ["pier_cafe", "riverside_walk"],
...
- `GET /companions/{id}/profile-outfit/latest`：返回该用户/角色最近一个未完成 profile outfit job，用于切页后恢复轮询；没有则返回 `{ "generation": null }`。
...
// Request
{
  ...
  "quick_action": null                  // 可选：scene_action / custom_scene_action / legacy gift
}
...
> ```json
> // event: quick_action_result
> { "ok": true, "item_id": "custom:draw a heart on the foggy window", "activity_id": "act_xxx", "memory_id": null, "cooldown_until": null }
> ```
> 校验失败会在开始流式前返回 JSON；无当前 scene 时返回 `quick_action_requires_scene`，catalog 动作不属于当前 scene 时返回 `quick_action_unavailable`，空/超长 custom action 返回 `invalid_quick_action`。如果上游模型因内容策略拒绝，返回/流式错误码使用 `content_filter`，Web 应显示“换一种描述”的明确提示。

> **邀约换场景（spec-036/037/038）：** 当请求带合法的 `invite_scene_id`（必须是已解锁场景，见下方 invite-targets），后端在 prompt 注入一段"用户邀请你前往 {场景}"的指令；本轮仍以**当前** `scene_id` 生成回复。回复结束后跑一次接受/拒绝判定，并通过 SSE 额外事件 `invite_result` 推送。`accepted: true` 只表示 companion 同意，不代表已经到达，也不要求前端立即切换 scene：
> ```json
> // event: invite_result
> { "accepted": true, "scene_id": "restaurant", "scene_art_url": "scenes/restaurant.png", "reason": "..." }
> ```
> Web 客户端收到 `accepted: true` 后创建 pending arrival，显示"立即到达 / 稍后"确认；用户点击立即到达后才切换 `scene_id` + 背景，并按需完成当前 activity。`accepted: false` 不切。越界/与关系阶段不符的邀约由现有每轮 signal 打分自然产生负向维度（distance/tension 上升），无独立惩罚逻辑。判定失败时回退 `accepted: false`（绝不误切）。
...
### `GET /companions/{companion_id}/invite-targets` *(spec-036)*

聊天内"邀请前往"浮窗的目的地候选。返回所有 `is_active = 1` 且 `unlock_condition` 对当前用户通过的场景；可用 `from_scene_id` 排除当前所在场景。关系门槛不到位的亲密场景（如设了高关系 `unlock_condition` 的场景）天然不出现在列表中。

```
// Query: ?from_scene_id=pier_cafe （可选）
// Response 200
{
  "targets": [
    { "id": "restaurant", "name": "Restaurant", "mood": "A warm everyday restaurant...", "art_url": "scenes/restaurant.png" },
    ...
  ]
}
```
