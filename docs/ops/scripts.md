...
> 新增一个 Worker 密钥 key 时,需同时改三处:`.env.example`(schema)、`generate-env-files.sh` 的 `WORKER_KEYS`、`upload-worker-secrets.sh` 的 `ALLOWED_WORKER_KEYS`。MiniMax TTS model / GroupId / voice catalog 不是 Worker 密钥,只改 `config/minimax-voices.<env>.json`。
