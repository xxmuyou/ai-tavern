...
### 2.5 本地开发（`.env.dev` 作为 SOT + `pnpm local-env:dev` 派生）
...
.env.dev (root, SOT, gitignored)
   │
   │ pnpm local-env:dev  ───────►  apps/app/.env.dev      （仅 EXPO_PUBLIC_*）
   │
   └───────────────────────►  infra/cloudflare/.dev.vars  （worker 消费 secrets，白名单）
```

派生文件 `apps/app/.env.dev` / `infra/cloudflare/.dev.vars` 都加 banner `# AUTO-GENERATED by scripts/prepare-local-env.sh — DO NOT EDIT`，**不要手改**。
...
```bash
pnpm local-env:dev              # dev：派生到 apps/app/.env.dev + infra/cloudflare/.dev.vars
pnpm local-env:dev --dry-run    # 只打印计划，不写文件
pnpm local-env:prod         # prod：仅派生 apps/app/.env.prod，prod worker secrets 走 pnpm cf:secrets:prod
pnpm dev                   # 自动准备本地 env，再启动 worker + Expo
```
