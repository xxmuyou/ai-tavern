...
### 2.5 本地开发（`.env.dev` 作为 SOT + `pnpm sync-env` 派生）
...
.env.dev (root, SOT, gitignored)
   │
   │ pnpm sync-env  ───────►  apps/app/.env.dev      （仅 EXPO_PUBLIC_*）
   │
   └───────────────────────►  infra/cloudflare/.dev.vars  （worker 消费 secrets，白名单）
```

派生文件 `apps/app/.env.dev` / `infra/cloudflare/.dev.vars` 都加 banner `# AUTO-GENERATED by scripts/sync-env.mjs — DO NOT EDIT`，**不要手改**。

**白名单维护：** `scripts/sync-env.mjs` 顶部 `WORKER_KEYS` 数组定义哪些 key 派生到 `.dev.vars`。新增 worker key 需同时改：
- `.env.example`（schema）
- `scripts/sync-env.mjs` 的 `WORKER_KEYS`

**常用命令：**

```bash
pnpm sync-env              # dev：派生到 apps/app/.env.dev + infra/cloudflare/.dev.vars
pnpm sync-env --dry-run    # 只打印计划，不写文件
pnpm sync-env:prod         # prod：仅派生 apps/app/.env.prod，prod worker secrets 仍走 wrangler secret put
pnpm dev                   # predev 自动跑 sync-env，再启动 worker + Expo
```
