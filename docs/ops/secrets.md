...
### 2.5 本地开发（根 `.env.*` 作为 SOT，本地只派生 Worker env）

> 详细 spec：[`docs/specs/spec-016-local-secrets-mgmt.md`](../specs/spec-016-local-secrets-mgmt.md)

**单一来源（Source of Truth）：** 根目录 `.env.local`（本地）、`.env.dev`（dev）和 `.env.prod`（prod）。它们都 gitignored，从 `.env.example` 复制模板填值。

**本地派生流程：**

```
.env.local (root, SOT, gitignored)
   │
   └─ pnpm generate:env:local ──► infra/cloudflare/.dev.vars
                                  （本地 worker 消费 secrets，白名单）
```

派生文件 `infra/cloudflare/.dev.vars` 加 banner `# AUTO-GENERATED by scripts/generate-env-files.sh — DO NOT EDIT`，**不要手改**。

不再生成 `apps/app/.env.*`。App/Web 标准命令通过 `scripts/tasks/run.sh` 直接读取根目录 `.env.local` / `.env.dev` / `.env.prod`，避免同一配置在多个位置重复。
...
```bash
pnpm generate:env:local           # 本地：派生 infra/cloudflare/.dev.vars
pnpm generate:env:local --dry-run # 只打印计划，不写文件
pnpm run:local                    # 自动准备本地 Worker env，再启动 worker + Expo
pnpm upload:secrets:dev           # dev/prod 远端 Worker secrets 走 upload:secrets:*
```

**新机器上手：**

1. `cp .env.example .env.local` → 填入本地值
2. `cp .env.example .env.dev` → 填入真实 dev 值
3. （可选）`cp .env.example .env.prod` → 填入 prod 值（用于 prod deploy/export）
4. `pnpm install` → 触发 husky 初始化 pre-commit hook
5. （可选）`brew install gitleaks` → pre-commit hook 会扫描，未装时 graceful skip
6. `pnpm run:local` → 一切就绪
