...
关系成长是渐进式玩法，不应让用户误以为需要立刻推进到亲密、私密或剧情高点。

UI 需要在 chat HUD、角色 Profile 的 Relationship goal、Unlocks 面板和 Story/Scene 提示里轻量解释：

- companion 会随着真实聊天、共同场景、故事进度和记忆逐步打开。
- favorite scene 和 active story progress 会放大本轮关系变化，但正负变化都会被放大；表现差、越界或过度推进也会更快增加 tension / distance。
- locked content 不是按钮失效，而是关系阶段未到；文案应提示用户继续自然建立关系。
- 提示只属于 UI 解释层，不进入 chat prompt，不让 companion 直接谈论“加分更快”。

推荐 UI 文案保持短句，例如 `Take your time. Bonds grow through real conversations, favorite scenes, and story progress.`，避免教育感或责备用户。
...
- Favorite scene 与 Story progress 的关系加速不叠加；两个条件同时满足时仍只应用一次。前端可用轻提示告诉玩家关系变化会被放大，但不应打断聊天，也不应暗示 Story mode 保证正向升温。
