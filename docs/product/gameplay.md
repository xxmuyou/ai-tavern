...
关系里程碑应沉淀为 memory，而不是只停留在聊天记录中。

| Memory | 触发示例 |
|---|---|
| `first_meeting` | 第一次见到 companion |
| `first_hangout` | 第一次完成 hang out |
| `first_date` | 第一次完成 date |
| `confession` | 浪漫阈值达成并完成告白事件 |
| `repair` | 成功修复一次 tension / hostility / distance |
| `anniversary` | 认识 N 天或互动 N 次 |

Memory 包含 companion、scene、日期、摘要、关键选择、关系变化和可选 `cg_url`。关键 CG 是里程碑奖励，不是每次聊天都出现。
...
| `show-engine.ts` Chapter 2（三场景约会） | **重构** —— 三场景固定 3 回合的设计要拆掉，改为"活动 + 场景 + 自由对话 + 事件"模型 |
| `show-engine.ts` Chapter 3 | **废弃** —— 固定章节制不适用，关系阶段由维度派生 |
