...
- **用户自创角色：** 用户填卡，私人体验；创建后可选择剧情包、用户自写 arc 或 AI 辅助草稿
...
关系阶段是通用进度，但只靠阶段名仍然不够“像游戏”。v1.x 增加一层以 companion 为主的轻量剧情拍（story beats）：每个 companion 可以挂一条 3-5 拍的线性 arc，scene 只是剧情发生的舞台，不再是体验主角。

**核心原则：**
- story beat 是通用框架，不绑定某一个官方角色；官方角色、自创角色、后续内容包都走同一套数据模型。
- 每个 beat 都是“当前这个 companion 为什么值得去见”的理由，包含 opener、objective、推荐 scene 和奖励。
- beat 可以由关系阶段、活动完成、事件选择或后续扩展条件解锁；v1.x 先做线性推进，不做复杂分支。
- 官方角色优先使用 authored beats；自创角色使用剧情包模板、用户轻量编辑或 AI 辅助生成的 3-5 拍 arc。
- UI-managed beats 默认由用户手动 `Mark as done` 完成，不让一轮聊天自动跳过剧情。旧自动完成只作为早期基础框架兼容。
- sandbox chat 仍然存在；story beat 只给方向和钩子，不强制用户只能点选项。
...
**场景入口呈现：**
- `scenes/enter` 返回在场 companion 时，应优先附带当前 active story beat。
- 前端优先展示 beat opener / objective；没有 active beat 时回退到普通 opener 和 daily state。
...
**自创角色剧情 setup：**
- 创建完成后给用户一个可跳过的 `Set up their story` 步骤：剧情包、自写、AI 辅助、稍后再说。
- 跳过后角色仍可自由聊天，但 Today / Scene / Chat 会优先提示如何补建 story arc，避免用户误以为没有后续目标。
- 发布公开自创角色时，story arcs 默认私有；用户可选择共享只读 arc，其他用户拥有独立进度。
...
- prompt 由规则拼接：最近聊天内容、行为/activity、scene、time slot、companion 外貌、relationship stage、emotion/status、active story beat。
