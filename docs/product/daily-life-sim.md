...
- 不做 3D、捏脸、建模、建造、开放地图
- 不做全局主线章节或按角色硬编码的固定剧情章节；轻量 companion story beats 由 gameplay/spec-026/spec-029 承接
...
- **默认偶遇边界**：`scene_id` 只能从 `is_active = 1`、非 intimate tier、且 `unlock_condition IS NULL` 的默认偶遇池选择。因为规则字段是全局共享缓存，不能按单个用户的解锁状态把 companion 随机放进锁定场景。
- `preferred_scenes` 只作为候选偏好，必须先过滤私密场景和带 `unlock_condition` 的场景；过滤后为空时，回退到默认偶遇池，而不是绕过解锁规则。
- 私密或有关系门槛的场景仍可通过用户主动进入、邀请、剧情转场或解锁 CTA 使用，但不参与默认 daily state 随机。
- Daily `scene_id` 是物理位置，不代表剧情模式。普通 companion 入口可以用 `Talk + Daily Scene` 打开 chat：显示地点和 scene action，但不注入 story beat；只有显式进入 `chat_mode=story` 且当前场景确实有 active story beat 时才使用剧情拍。
- Daily/Scene roster 不从全站 public 或 online companion 池补位；无头像 companion 从 scene 玩法里过滤后，场景可以暂时显示更少 companion。

**官方 companion 行为：**
- 每个 slot 按规则在过滤后的 `preferred_scenes` + 场景 `default_companions` 里加权选位置；候选为空时从默认偶遇池稳定随机
...
| 情况 | 行为 |
|---|---|
| 用户填了 `preferred_scenes` | 过滤私密/锁定场景后，在剩余 preferred_scenes 里按 slot 轮转 |
| 用户没填 `preferred_scenes` 或过滤后为空 | 从默认偶遇池稳定随机 |
...
"章节"在本产品中不表示全局主线关卡。体验节奏由关系阶段、关系目标、活动和轻量 story beats 共同提供：关系阶段告诉用户“关系到了哪里”，story beats 告诉用户“这个 companion 当前有什么事值得继续”。
...
- 不展示为固定主线章节列表，避免用户以为游戏有通关式剧情
- 在 Story mode 或 Story 专属入口中有 active story beat 时，UI 优先展示 beat objective；没有 active beat 时不编造剧情，Talk / daily surfaces 回退到关系目标和 daily state
