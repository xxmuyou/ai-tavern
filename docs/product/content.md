...
本文档的 v1 baseline 仍以通用关系阶段和日常活动为主，不写全局主线章节。当前 v1.x 路线从 companion story beats 过渡到 Scene stories：官方内容可维护 scene preset stories，用户可在 Scene 页面通过 [`spec-029`](../specs/spec-029-user-created-story-arcs.md) 的模板、自写和 AI 辅助能力创建私有 story。
...
- [x] 是否提供少量"场景故事线"：做轻量 Scene stories，不做全局主线章节；官方 preset stories + 用户在 Scene 页面走剧情包 / 自写 / AI 辅助（见 spec-026 / spec-029 / spec-040）
